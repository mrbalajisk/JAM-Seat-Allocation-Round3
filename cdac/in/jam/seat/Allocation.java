...
public class Allocation{
		...
		void lastRoundAllocation(){
				...
				for(Applicant applicant: applicants){
						...
						if( lastRoundApplicant.statusId == 3 || lastRoundApplicant.statusId == 4){
		...
		boolean allocate(String type, Course course, Applicant applicant, int choiceNo, List<FreeSeat> freeSeats, boolean flagSuperNumeri){
		...
		boolean allocation(String paper, List<Applicant> applicants, Map<String, Course> courses, boolean isSuperNumeri){
						...
						for( int choiceNo = 0; choiceNo < applicant.validChoices.length && choiceNo <= applicant.allocatedChoice; choiceNo++){
								...
								if( course != null ){
										...
										if( choiceNo == applicant.allocatedChoice ){           /* same-choice  */
												...
												if( allocate("GEN", course, applicant, choiceNo, freedSeats, isSuperNumeri ) ){
														flag = true;
												...
												if( allocate("GEN", course, applicant, choiceNo, freedSeats, isSuperNumeri ) ){
														flag = true;
														break;      
												}else if( ! applicant.category.equals("GEN") && allocate(applicant.category, course, applicant, choiceNo, freedSeats, isSuperNumeri ) ){
														flag = true;
														break;      
												}else if( applicant.isPd  && allocate( applicant.category+"PWD", course, applicant, choiceNo, freedSeats, isSuperNumeri ) ){
														flag = true;
														break;      
												}
										}
						...
						for(Applicant applicant: applicants){
								...
								for(int i = 0; i < ( applicant.allocatedChoice - 1) && i < applicant.validChoices.length; i++){
										...
										if( course != null ){
                                                ...
                                                if( applicant.isPd &&  ( course.quotas.get( applicant.category+"PWD").originalSeat > course.quotas.get( applicant.category+"PWD").seat ) ){
                                                        count++;
														System.out.println("#"+count+". "+applicant.applicationId+", "+paper+", "+course.programCode+", "+applicant.category+"PWD , "+course.quotas.get( applicant.category+"PWD" ).closingRank+", "+applicant.ranks.get(paper).rank);
                                                        
                                                }

                                                if( applicant.category.equals("OBC") 
                                                  &&  ( course.quotas.get("OBC").originalSeat > course.quotas.get("OBC").seat ) ){
														count++;
														System.out.println("#"+count+". "+applicant.applicationId+", "+paper+", "+course.programCode+", "+applicant.category+", "+course.quotas.get( applicant.category ).closingRank+", "+applicant.ranks.get(paper).rank);
		...
		private boolean obcSeatConversion(){
		...
		private boolean pwdSeatConversion(){
				Set<String>  papers = coursesMap.keySet();
                boolean convert = false;
				for(String paper: papers){
						Map<String, Course>  courses = coursesMap.get(paper);
		...
		private void allocation(int iterration, boolean isSuperNumeri, boolean obcConvert){

				System.out.println("Global Interation: "+iterration);	
				System.err.println("Global Interation: "+iterration);	

				paperWiseallocation( false );	
                ...
                while( convert ){ 
                    convert = false;    
                    if( pwdSeatConversion() ) {
				        paperWiseallocation( isSuperNumeri );
                        convert = true;  
                    }     
                }    
                ...
                while( convert && obcConvert ){ 
                    convert = false;    
                    if( obcSeatConversion() ) {
				        paperWiseallocation( isSuperNumeri );
                        convert = true;  
                    }     
                }    
				...
				if( SuperNumeriReduced && readjustment() ){
						allocation( iterration + 1, isSuperNumeri, obcConvert ); 	
				}
		}

		void paperWiseallocation( boolean flagSuperNumeri ){
				...
				while( flag ){
						...
						for(String paper: papers){
								...
								if( allocation( paper, paperWiseApplicant.get(paper), coursesMap.get( paper ), flagSuperNumeri ) ){
										flag = true;				
								}
		...
		private void notInDraft(){
				...
				int count = 0;
				System.out.println("---- New Applicant Program-wise ---");
				...
				System.out.println("---- Applicant Move out Program-wise ---");

				for(String program: programs){
						List<String> mapplicants =  draftAllocation.get( program ).moveout;
						boolean flag = true;
						if( mapplicants.size() > 0 ){
								System.out.print(program+": ");
		...
		public static void main(String[] args){
				try{
						...
						boolean isSuperNumeri = false;
						boolean obcConvert = false;
						int i = 0;

						while( i < args.length ){
								if( args[i].equals("-isup") ){
										isSuperNumeri = true;
								}else if( args[i].equals("-obccnv") ){
										obcConvert = true;
										...
										System.out.println("-obccnv :  OBC seat conversion for allocation [defalut false] " );
